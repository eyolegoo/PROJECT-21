...
![alt text](<23 Create the ClusterRole.png>)
...
![alt text](<24a configure Role Based Access (RBAC) for Kubelet Authorization.png>)
...
![alt text](<24b configure Role Based Access (RBAC) for Kubelet Authorization.png>)
...
- To mitigate security risks and have a better controlled network topology, Kubernetes uses [CNI (Container Network Interface)](https://github.com/containernetworking/cni) to manage [Network Policies](https://kubernetes.io/docs/concepts/services-networking/network-policies/) which can be used to operate the Pod network through external plugins such as [Calico](https://docs.tigera.io/calico/latest/about), [Flannel](https://github.com/flannel-io/flannel#flannel) or [Weave Net](https://www.weave.works/oss/net/) to name [a few](https://kubernetes.io/docs/concepts/cluster-administration/networking/). With these, you can set policies similar to how you would configure segurity groups in AWS and limit network communications through either cidr `ipBlock`, `namespaceSelector`s, or  `podSelectors`, you will see more of these concepts further on.
